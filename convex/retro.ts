    ...
    return await ctx.db
      ...
      .withIndex("sessionAndParticipant", (q) =>
        q
          ...
          .eq("participantName", args.participantName),
      )
      .collect();
  },
});
...
export const getActionItems = query({
  ...
  handler: async (ctx, args) => {
    return await ctx.db
      .query("actionItems")
      .withIndex("createdInSession", (q) =>
        q.eq("createdInSession", args.sessionId),
      )
      .collect();
  },
});
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      ...
      if (existing.sessionToken === args.sessionToken) {
        ...
        // Token mismatch
        throw new Error(
          "Participant with this name already exists in this session",
        );
      }
    }
