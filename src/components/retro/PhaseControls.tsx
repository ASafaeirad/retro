...
const PHASE_FLOW: { phase: Phase; label: string; description: string }[] = [
  { phase: 'REVIEW_ACTIONS', label: 'Review Actions', description: 'Review previous sprint action items' },
  { phase: 'ADD_TICKETS', label: 'Add Tickets', description: 'Add tickets to the board' },
  { phase: 'PRESENT', label: 'Present', description: 'Present tickets one by one' },
  { phase: 'GROUP', label: 'Group', description: 'Group related tickets' },
  { phase: 'VOTE', label: 'Vote', description: 'Vote on tickets and groups' },
  { phase: 'DISCUSS', label: 'Discuss', description: 'Discuss voted items with timer' },
  { phase: 'COMPLETED', label: 'Complete', description: 'Mark retro as completed' },
]

export function PhaseControls({ currentPhase, onPhaseChange, canAdvance = true, className }: PhaseControlsProps) {
  ...
  return (
    <div className={cn('rounded-lg border border-[var(--line)] bg-[var(--surface)] p-4', className)}>
      <div className="mb-4">
        <h3 className="text-sm font-semibold text-[var(--sea-ink)]">Workflow Control</h3>
      ...
      <div className="mb-4 rounded-md border-2 border-[var(--lagoon)] bg-[var(--chip-bg)] p-3">
        <div className="text-xs text-[var(--kicker)] font-medium mb-1">Current Phase</div>
